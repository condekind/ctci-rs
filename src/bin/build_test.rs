...
use chrono::Utc;
use syn::{self, visit::Visit, ExprArray, ItemStatic, ItemConst};
...
struct SliceVisitor {
    ...
    // of our defined `visit_expr_array` for the ones inside `ItemStatic`s.
    // There's more info in the comments of `visit_expr_array`
    //
...
impl<'ast> Visit<'ast> for SliceVisitor {
    fn visit_expr_array(&mut self, expr: &'ast ExprArray) {
        ...
        if self.inside_item.0 {
            self.slices.insert(self.inside_item.1.clone(), expr.elems.len());
            println!("Length of {}: {}", self.inside_item.1, expr.elems.len());
        }
    }

    fn visit_item_const(&mut self, item: &'ast ItemConst) {
        println!("Found ItemConst named {:?}\n", item.ident);
